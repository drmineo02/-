...
// دالة لضبط موقع وأبعاد quran-text ديناميكيًا
function adjustQuranTextLayout() {
    const quranText = document.getElementById("quran-text");
    const audioPlayer = document.querySelector(".audio-player");
    const sidebar = document.getElementById("sidebar");
    const mainContent = document.querySelector(".main-content");

    if (quranText && audioPlayer) {
        ...
        // إعادة ضبط الحجم إذا كان quran-text نشطًا
        if (quranText.classList.contains("active")) {
...
function adjustFontSize(ayahElement) {
    ...
    while ((ayahElement.scrollWidth > containerWidth || ayahElement.scrollHeight > containerHeight) && fontSize > 36) {
        fontSize -= 1;
        ayahElement.style.fontSize = `${fontSize}px`;
    }

    // زيادة الحجم تدريجيًا إذا كان هناك مساحة إضافية
    while ((ayahElement.scrollWidth < containerWidth * 0.9 && ayahElement.scrollHeight < containerHeight * 0.9) && fontSize < 240) {
        fontSize += 1;
        ayahElement.style.fontSize = `${fontSize}px`;
...
function highlightAyah(ayahNumber) {
    ...
    if (activeAyah) {
        ...
        adjustFontSize(activeAyah);
        adjustQuranTextLayout(); // تحديث التصميم عند تغيير الآية
    } else {
